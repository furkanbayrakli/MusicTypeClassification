...
def train_and_evaluate(data_path, description):
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(data['filepath'], data['label_encoded'])):
        print(f"\nModel DenseNet121 - Training on {description} - at Fold {fold + 1}/{num_folds}")
        ...
        model = build_densenet121_model(input_shape=(image_size[0], image_size[1], 3))
...
def log_data_split(train_data, val_data, test_data, fold_number, log_file="seperatedDataDense.txt"):
...
# print(f"\nDenseNet Mel Spectrogram Metrics:\n{mel_metrics}")

# spec_metrics = train_and_evaluate(spectrogram_path, "regular spectrograms")
# print(f"DenseNet Regular Spectrogram Metrics:\n{spec_metrics}")
